...
def generate_finance_dataset(n_samples=1000):
    ...
    # Scale and rename for finance context
    X[:, 0] = X[:, 0] * 100 + 650  # credit score ~650-750
    X[:, 1] = X[:, 1] * 20  # income volatility
    return X, y
...
def plot_decision_boundary(model, X, y, epoch, filename, true_boundary_func=None):
    ...
    if true_boundary_func:
        ...
        plt.legend(["NN boundary", "True boundary"], loc="upper right")
    plt.tight_layout()
